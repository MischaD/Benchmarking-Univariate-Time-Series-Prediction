# FROM: https://github.com/hfawaz/cd-diagram/blob/master/main.py
...
def graph_ranks(avranks,
    ...
    for i in range(math.ceil(k / 2), k):
        ...
        line([(rankpos(ssums[i]), cline), (rankpos(ssums[i]), chei),
             ...
             linewidth=linewidth)
        if labels:
            text(textspace + scalewidth - 0.3,
                 chei - 0.075,
                 format(ssums[i], '.4f'),
                 ha="left",
                 va="center",
                 size=10)
        text(textspace + scalewidth + 0.2,
             chei,
             filter_names(nnames[i]),
             ha="left",
             va="center",
...
def draw_cd_diagram(df_perf=None,
                    alpha=0.05,
                    title=None,
                    labels=False,
                    output_path="cd-diagram.png"):
    """
    ...
    graph_ranks(average_ranks.values,
                average_ranks.keys(),
                p_values,
                cd=None,
                reverse=True,
